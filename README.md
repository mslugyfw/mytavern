...
基于世界书的交互式角色扮演系统，适用于 OpenClaw、Claude Code、Cursor 等智能体平台。核心是纯Python脚本，只要智能体能执行命令就能跑。

> 💡 不同智能体能力不同。如果缺少某个功能（没有LCM、没有生图、没有搜索），智能体会自动找替代方案。也可以直接告诉它"xxx功能不好用，帮我改一下"，让它自己造轮子。
